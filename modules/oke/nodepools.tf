resource "oci_containerengine_node_pool" "nodepools" {
  ...
  dynamic "freeform_tags" {
    for_each = lookup(each.value, "freeform_tags", "") != "" ? each.value.freeform_tags : {}
    content {
 ...
 */
  ...
  node_config_details {
    ...
    dynamic "node_pool_pod_network_option_details" {
      ...
      content {
        ...
        pod_subnet_ids    = tolist([var.subnets["pods"]])
      }
    }
  ...
  dynamic "node_source_details" {
    ...
    content {
      boot_volume_size_in_gbs = lookup(each.value, "boot_volume_size", 100)
